...
def Main():
    global current, images, fadeframes
    #       [i%numscreens] ?
    while True:
        if current == 0:
            LoadFiles()
        ...
        #while alpha <= 1.0:
        for i in range(0, numscreens):
            # print(current, current + i, current + i + 1)

            alpha = 0.0
            for x in range(0, fadeframes):
                # print(i, x)
                alpha += 1.0 / fadeframes

                blend[i][x] = ImageTk.PhotoImage( Image.blend( images[(current + i) % len(files)], images[(current + i + 1) % len(files)], alpha ) )

        for x in range(0, fadeframes):
            for i in range(0, numscreens):
                canvas[i].itemconfig(canvasimage[i], image=blend[i][x])
            time.sleep(framerate)
            root.update()
